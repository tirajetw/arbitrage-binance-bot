...
api_key = 'f7xQybTWPrY20G1NYoW9AE8MF1C5qPk0lS9l7sWUSgWeUaWYBKYVQJLBySnYtkmF'
api_secret = 'E8jtxPvGiztGlgN9UjSAiO7eNZqI8kpFDYwMJROmtJXf5vonRF02IdCW9yE4Ow9t'
...
def initialize_arb():
    ...
    try:
        ...
        m=0
        #tickers = client.get_orderbook_tickers()
        #arbitrage_bin(list_of_arb_sym[0], tickers, 1, 1)
...
def profit_cal(list_of_sym):
    ...
    if final_amount_adj > start_amount :
        tri_arb_paper_msg = "(*) Starting Amount: "+str(start_amount)+str(list_of_sym[0][0:3])+" "+'\n'
        #Buy Currency 2 with Currency 1
        tri_arb_paper_msg += "\n"+str(list_of_sym[0][0:3])+"->"+str(list_of_sym[0][3:6])
        ...
        tri_arb_paper_msg += "\n"+str(list_of_sym[0][3:6])+"->"+str(list_of_sym[1][0:3])
        ...
        tri_arb_paper_msg += "\n"+str(list_of_sym[1][0:3])+"->"+str(list_of_sym[0][0:3])
        ...
        print('(*) normal solution profit = {} BNB\n\n'.format(final_amount_adj-start_amount))
        print('Place first order : '+str(list_of_sym[0][0:3])+"->"+str(list_of_sym[0][3:6]))
        
        """1ST TRADE"""
        print("order1 = client.order_limit_sell(\n\tsymbol=\'"+str(list_of_sym[0][0:3])+str(list_of_sym[0][3:6])+'\',\n\t'+'quantity='+str(start_amount)+',\n\t'+'price=\''+str(float(exch_rate_list[0]))+'\')\n')
        client.order_limit_sell(
            symbol=str(list_of_sym[0][0:3])+str(list_of_sym[0][3:6]),
            quantity=start_amount,
            price=str(float(exch_rate_list[0])))  
        while bool(client.get_open_orders(symbol=str(list_of_sym[0][0:3])+str(list_of_sym[0][3:6]))):
            print('.')
        ...
        while bool(client.get_open_orders(symbol=str(list_of_sym[1][0:3])+str(list_of_sym[0][0:3]))):
            print('.')
        print('(*) Finish Arbitrage!')
        
    else:
        print('\n(*) reverse solution calculate...\n')
        amt_coin2 = start_amount / float(exch_rate_list[1])
        amt_coin2_no_fee = amt_coin2
        amt_coin2_fee = amt_coin2*fee
        amt_coin2_adj = amt_coin2*(1-fee)
        amt_coin3 = amt_coin2_adj * float(exch_rate_list[2])
        ...
        amt_coin3_fee = amt_coin3 * fee
        amt_coin3_adj = amt_coin3*(1-fee)
        final_amount = amt_coin3_adj / float(exch_rate_list[0])
        final_amount_no_fee = amt_coin3_no_fee / float(exch_rate_list[0])
        final_amount_fee = final_amount *fee
        final_amount_adj = final_amount *(1-fee)
        if final_amount_adj > start_amount :
            tri_arb_paper_msg = "(*) Starting Amount: "+str(start_amount)+str(list_of_sym[0][0:3])+" "+'\n'
            ...
            tri_arb_paper_msg += "\nFinal Amount Adjusted: "+str(list_of_sym[0][0:3])+" "+str(final_amount_adj)+'\n'
            print(tri_arb_paper_msg)
            print('(*) reverse solution profit = {} BNB\n\n'.format(final_amount_adj-start_amount))
            time.sleep(5)
            """1ST TRADE"""
            ...
            print('(*) Cant Arbitrage!')

def profit_cal_re(list_of_sym):
    bnb_balance = float(int(float(client.get_asset_balance(asset='BNB')["free"])*100)/100)
    #print("start_amount = {} BNB".format(bnb_balance))
    start_amount = 1
    print('\n(*) reverse solution calculate...\n')
...
def arbitrage_bin(list_of_sym, tickers, cycle_num=1, cycle_time=30, place_order='Yes', real_order='Yes'):
    ...
    for k in range(0,cycle_num):
                ...
                print(sell_price)'''
                #print(list_of_sym[0][0:3]) #BNB
                #print(list_of_sym[0][3:6]) #BTC
                #print(list_of_sym[1][0:3]) #...


                """if float(rate1)<float(rate2):
                    arb_profit = ((rate2-rate1)/rate2)*100
                    print('arb_profit = {}'.format(arb_profit))
                if float(rate1)>float(rate2):
                    arb_profit = ((rate1-rate2)/rate1)*100
                    print('(inverse) arb_profit = {}'.format(arb_profit))
                    """
...
initialize_arb()
tickers = client.get_orderbook_tickers()
arbitrage_bin(list_of_symbols18, tickers, 1, 1)
#profit_cal_re(list_of_symbols18)
"""